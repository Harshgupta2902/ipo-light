...
export async function generateMetadata() {
    ...
    const pathname = completepathname?.replace('/pincode-finder/', "");

    const segments = pathname?.split('/');
    const taluk = segments?.[segments.length - 2] || "";
    const po = segments?.[segments.length - 1] || "";


    let data = null;
    try {
        const response = await fetchPincodeData(taluk, po);
        data = response[0];
        console.log(data);
    ...
    const metaTitle = `${data.Address ? data.Address : ""}`;
    const metaDescription = `${data.Description ? data.Description : ""}`;
    const keywords = `${data.PostOffice}, ${data.RelatedSubOffice}, ${data.RelatedHeadOffice}, ${data.State}, ${data.District}, ${data.PostalCircle}, ${data.Taluk}`;
    return {
        title: metaTitle,
...
const fetchPincodeData = async (taluk: string, po: string) => {
    try {
        console.log(`taluk: ${taluk}______ po: ${po}`);

        const response = await fetch(`${endpoints.getPincode}?taluk=${taluk}&po=${po} `);
        if (!response.ok) {
    ...
    return (
        <div className="container">
            ...
            <div className=" container relative overflow-x-auto sm:rounded content">
                <h1 className="text-2xl" >{data.PostOffice}, {data.RelatedSubOffice}, {data.State}</h1>
                <p className="text-md" >PinCode Code of {data.PostOffice} is {" "}
                    <strong>{data.PinCode}</strong>
                </p>
                <p className="text-md pb-6" >Find More Details of {data.PostOffice} Post Office, {data.RelatedSubOffice} {data.State}, {data.STATE} below:</p>
